...
%     at = LS_init_abs_thr(samp_freq, p0_level_dB, fb);
    ...
    bm_b = zeros(num_block, 1); %M/S flags
...
%         top_band = fb(ls_idx).top_band;
        ...
        for widx = 1:num_win
            ...
            SMR_dB_bands = mc_tonality(X_pow_bins, freq_band_top,fb_per_cb,z_max);%<<===add ,fb_per_cb,z_max
            ...
            if (num_chan == 2)
                %stereo only==============================================
                [ms_flags, X(k+1:k+Nc,:), mdct_qs_bands_hat] = ...
                   ms_encode(X(k+1:k+Nc,:), mdct_qs_bands_hat, ...
                    Xs_pow_bands, freq_band_top);
                 fprintf('%d', ms_flags); fprintf('\n');
                if (bno>0); ms_b(bno) = bm_b(bno) + length(ms_flags); end
            end
            %expand quantizer stepsizes from bands to bins
            ...
            if (quant_mode == 0)
                %no quantization
                Y(k+1:k+Nc,:) = X(k+1:k+Nc,:);
            else
                %use quantized coefficients
                Y(k+1:k+Nc,:) = X_hat(k+1:k+Nc,:);
            end
            ...
            if (num_chan == 2)
                %stereo only
                Y(k+1:k+Nc,:) = ...
                    ms_decode(Y(k+1:k+Nc,:), ms_flags, freq_band_top);
            end
